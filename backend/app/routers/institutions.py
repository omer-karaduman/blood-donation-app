...
router = APIRouter(prefix="/institutions", tags=["Kurum ve Hastane İşlemleri"])


@router.get("/", response_model=List[schemas.InstitutionResponse])
def get_institutions(
    district_id: Optional[uuid.UUID] = None,
    tipi: Optional[models.InstitutionTypeEnum] = None,
...
def create_institution(inst_in: schemas.InstitutionCreate, db: Session = Depends(get_db)):
    """Yeni bir hastane veya kan merkezi ekler."""
    ...
    if inst_in.latitude is not None and inst_in.longitude is not None:
        new_inst.konum = WKTElement(
            f"POINT({inst_in.longitude} {inst_in.latitude})", srid=4326
        )
    db.add(new_inst)
    db.commit()
    db.refresh(new_inst)
...
def get_institution_staff(institution_id: uuid.UUID, db: Session = Depends(get_db)):
    """Belirli bir kuruma kayıtlı personelleri listeler."""
    staff_list = (
        db.query(models.StaffProfile)
        .filter(models.StaffProfile.kurum_id == institution_id)
        .all()
    )
    return [
        {
