...
router = APIRouter(prefix="/users", tags=["Kullanıcı Profili"])


@router.get("/{user_id}/profile")
def get_user_profile_data(user_id: uuid.UUID, db: Session = Depends(get_db)):
    """Kullanıcının id'sine göre Ad/Soyad, Kan Grubu veya Unvan bilgisini döner."""
    ...
    if user.role.name == "DONOR" and user.donor_profile:
        return {
            ...
            "mahalle":   (
                user.donor_profile.neighborhood.name
                if user.donor_profile.neighborhood
                else "Belirtilmemiş"
            ),
        }
    ...
    if user.role.name == "staff" and user.staff_profile:
        return {
            ...
            "kurum_adi":  (
                user.staff_profile.institution.kurum_adi
                if user.staff_profile.institution
                else "Bilinmiyor"
            ),
        }
